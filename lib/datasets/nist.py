...
class nist(imdb):
  def __init__(self, image_set, year, dist_path=None):
    ...
    self._image_set = image_set.split('dist_')[1]  #'NIST_train_new_2'

    self._dist_path = dist_path #NIST '/media/li/Li/NIST_new'
    self._data_path=self._dist_path   #NIST '/media/li/Li/NIST_new'
    ...
    self._image_ext = {'.png'}
    self._image_index = self._load_image_set_index()   #txt文件每一行
  ...
  def image_path_at(self, i):
    ...
    """
    return self.image_path_from_index(os.path.splitext(self._image_index[i].split(' ')[0])[0])   #返回图像名称  无拓展名,调用下面函数，返回图像绝对路径，返回图像的绝对路径/.../.../...png
  def mask_path_at(self, i):
    """
    Return the absolute path to image i in the image sequence.image name
    """
    return self.mask_path_from_index(os.path.splitext(self._image_index[i].split(' ')[0])[0])   #返回图像名称  无拓展名,调用下面函数，返回图像绝对路径，返回图像的绝对路径/.../.../...png
  ...
  def mask_path_from_index(self, index):
    ...
    for ext in self._image_ext:
      #image_path = os.path.join('/home-3/pengzhou@umd.edu/work/xintong/medifor/portrait/test_data',
                              #index + ext)
      mask_path = os.path.join('/media/li/Li/NIST2016/mask',
                              index + ext)
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      rec, prec, ap = voc_eval(
        filename,filename2, annopath, imagesetfile, cls_ind, cachedir, self._class_to_ind,ovthresh=0.5,
