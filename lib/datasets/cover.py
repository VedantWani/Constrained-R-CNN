...
class dist_fake(imdb):
  def __init__(self, image_set, year, dist_path=None):
    ...
    self._data_path=self._dist_path+'probe'
  ...
  def mask_path_from_index(self, index):
    ...
    """
    im_id=index.split('_')[1]
    mask_name=im_id+'forged'
    for ext in self._image_ext:
      #image_path = os.path.join('/home-3/pengzhou@umd.edu/work/xintong/medifor/portrait/test_data',
                              #index + ext)
      mask_path = os.path.join('/media/li/Li/cover/mask',
                              mask_name + ext)
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join(self._dist_path,
                                  self._image_set + '.txt')
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      for i in range(len(rec)):
        print((2*rec[i]*prec[i])/(rec[i]+prec[i]))
