...
from nets.network_mask import Network
...
class resnet_C3Rcbam(Network):
  ...
  def cbam_module(self,inputs, reduction_ratio=0.5, name=''):
      with tf.variable_scope("cbam_" + name, reuse=None):
          ...
          conv_layer = slim.conv2d(max_avg_pool_spatial, 1, [7, 7], padding="SAME",
  ...
  def get_variables_to_restore(self, variables, var_keep_dic):
    ...
    for v in variables:
      ...
      if v.name == (self._resnet_scope + '/conv1/weights:0'):
        ...
        continue
      elif len(v.name.split('/'))>1:
        if v.name.split('/')[1]=='cls_score':
            continue
        elif v.name.split('/')[1]=='bbox_pred':
            continue
