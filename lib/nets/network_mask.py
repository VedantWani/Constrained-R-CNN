...
from layer_utils.proposal_mask_layer import proposal_mask_layer
...
class Network(object):
    ...
    def _add_mask_gt_summary(self, mask):
        mask1=mask*255.0

        return tf.summary.image('mask_gt', mask1)
    def _add_maskbg_summary(self, mask):


        return tf.summary.image('maskbg', mask,80)
    def _add_maskfg_summary(self, mask):


        return tf.summary.image('maskfg', mask,80)
    ...
    def _mask_vis(self,mask_pre,mask_target):
        mask_target=mask_target
        mask_pre = tf.where(mask_pre < 0.5, x=tf.zeros_like(mask_pre), y=tf.ones_like(mask_pre))
        mask_target = tf.where(mask_target < 0.5, x=tf.zeros_like(mask_target), y=tf.ones_like(mask_target))


        mask_pre = tf.image.resize_images(mask_pre, [200, 200])
        mask_target = tf.image.resize_images(mask_target, [200, 200])
        # mask_pre = tf.where(mask_pre < 0.5, x=tf.zeros_like(mask_pre), y=tf.ones_like(mask_pre))
        # mask_target = tf.where(mask_target < 0.5, x=tf.zeros_like(mask_target), y=tf.ones_like(mask_target))
        mask_pre = mask_pre * 255.0
        mask_target = mask_target * 255.0
        splice = tf.ones_like(mask_pre)
        splice=splice*255.0
        mask= tf.concat(2,[mask_pre, splice, mask_target])
        return mask
    ...
    def create_architecture(self, sess, mode, num_classes, tag=None,
        ...
        val_summaries = []
        masktg_fg = self._mask_vis(self._predictions["mask_softmaxfg"],
                                                              self._proposal_targets['mask_targetfg'])
        masktg_bg=self._mask_vis(self._predictions["mask_softmaxbg"],
                                                              self._proposal_targets['mask_targetbg'])
        with tf.device("/cpu:0"):
            ...
            if cfg.USE_MASK is True:
                val_summaries.append(self._add_mask_gt_summary(self._proposal_targets['mask_gt']))
                val_summaries.append(self._add_maskfg_summary(masktg_fg))
                val_summaries.append(self._add_maskbg_summary(masktg_bg))
    ...
    def test_image(self, sess, image, noise, im_info):
        ...
        s = sess.run(fetches=['resnet_v1_101/cls_score/weights:0'],
                     feed_dict=feed_dict)

        cls_score, cls_prob, bbox_pred, rois, feat,s,mask_sigmoid,mask_data,layers = sess.run(
            [self._predictions["cls_score"],
             self._predictions['cls_prob'],
             self._predictions['bbox_pred'],
             self._predictions['rois'],
             self._layers['fc7'],
             'resnet_v1_101/cls_score/weights:0',
             self._predictions['mask_softmaxfg'],
             self._proposal_targets['mask_data'],
             self._layers],
            feed_dict=feed_dict)

        return cls_score, cls_prob, bbox_pred, rois, feat,  s, mask_sigmoid,mask_data,layers
    ...
    def train_step(self, sess, update_weights, update_biases, blobs, train_op):
        ...
        feed_dict = {self._image: blobs['data'], self.noise: blobs['noise'], self._im_info: blobs['im_info'],
                     self._gt_boxes: blobs['gt_boxes'],self._mask: blobs['mask']}
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss, temp, temp1, temp2 = sess.run(
    ...
    def train_step_without_mask(self, sess, update_weights, update_biases, blobs, train_op):
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, temp, temp1, temp2 = sess.run(
            [self._losses["rpn_cross_entropy"],
             self._losses['rpn_loss_box'],
             self._losses['cross_entropy'],
             self._losses['loss_box'],
    ...
    def train_step_with_summary(self, sess, update_weights, update_biases, blobs, train_op):
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss, summary, temp, temp1, temp2,mask_out = sess.run(
            [self._losses["rpn_cross_entropy"],
             ...
             self._losses['total_loss'],
             self._summary_op,
             train_op, update_weights, update_biases,self._predictions['mask_out'] ],
            feed_dict=feed_dict)
    ...
    def train_step_with_summary_without_mask(self, sess, update_weights, update_biases, blobs, train_op):
        feed_dict = {self._image: blobs['data'], self.noise: blobs['noise'], self._im_info: blobs['im_info'],
                     self._gt_boxes: blobs['gt_boxes']}
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, summary, temp, temp1, temp2 = sess.run(
            [self._losses["rpn_cross_entropy"],
             self._losses['rpn_loss_box'],
             self._losses['cross_entropy'],
             self._losses['loss_box'],
